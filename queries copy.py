...
# ============================== Baseline Model ==============================
def baseline_improve_chunk(chunk_text: str) -> str:
    """Baseline helper that asks the model to improve a chunk without conflict context."""
    prompt = f"""
Solve the inconsistencies if you see they are there. If they are not there, just return the story as is.

If there is an inconsistency, just return the improved sentence, no other text.

SO NO OTHER TEXT, JUST THE IMPROVED SENTENCE OR THE SAME SENTENCE IF THERE IS NO INCONSISTENCY.
...
"""
    try:
        resp = ollama.chat(model="llama3:latest", messages=[{"role": "user", "content": prompt}])
        improved = resp.get("message", {}).get("content", "").strip()
        return improved if improved else chunk_text
    except Exception:
        return chunk_text

def evaluate_baseline(chunks: List[str]) -> Dict[str, Any]:
    """
    Run the baseline improve-only prompt across all chunks and compute evaluation metrics.
    Returns the improved chunks alongside metrics, violation counts, and reputation scores.
    """
    print("\n=== Node: Baseline Improve ===")
    improved_chunks: List[str] = []
    for i, chunk_text in enumerate(chunks or []):
        improved = baseline_improve_chunk(chunk_text)
        improved_chunks.append(improved)
        print(f"baseline_chunk_{i}: {improved}")

    baseline_state: InbetweenState = {
        "story": " ".join(improved_chunks),
    ...
    baseline_state = extract(baseline_state)
    baseline_state = check_conflicts(baseline_state)

    y_true, y_pred, aligned = collect_labels(baseline_state)
    metrics = compute_metrics(y_true, y_pred)
    violations = sum(y_true)
    reputation = reputation_scores(aligned, y_true)
...
def run_one_story(story_text: str) -> Dict[str, Any]:
    ...
    baseline_eval = evaluate_baseline(list(state.get("chunks", [])))
    mbaseline = baseline_eval["metrics"]
    vb_baseline = baseline_eval["violations"]
    baseline_story = " ".join(baseline_eval.get("chunks", []))
...
def run_suite(stories: List[str]):
    ...
    micro_baseline = micro_sum((r["baseline"] for r in runs))
    micro_after  = micro_sum((r["after"]  for r in runs))
    viol_before  = sum(int(r["viol_before"]) for r in runs) or 0
    viol_baseline = sum(int(r["viol_baseline"]) for r in runs) or 0
    viol_after   = sum(int(r["viol_after"])  for r in runs) or 0
    baseline_resolution = _safe_ratio((viol_before - viol_baseline), viol_before) if viol_before else 0.0
    ...
    print("Baseline:", micro_baseline)
    print("After   :", micro_after)
    print(
        f"Violations (gold): {viol_before} → {viol_baseline} → {viol_after}  "
        f"(Baseline Δ: {baseline_resolution:.3f}, Final Δ: {resolution:.3f})"
    )
...
When Amelia came in, she immediately bought two croissants which were made in the oven.""",
    ...
    def run_full_once(story_text: str, label: str):
        ...
        baseline_eval = evaluate_baseline(list(state.get("chunks", [])))
        print("\n=== METRICS — BASELINE (Improve prompt) ===")
        print(baseline_eval["metrics"])
        print(f"Gold violations (sum of y_true): {baseline_eval['violations']}")
        _print_reputation("BASELINE", baseline_eval["reputation"], top_k=5)
        baseline_story = " ".join(baseline_eval.get("chunks", []))
        print("\n=== BASELINE OUTPUT ===")
        print(baseline_story if baseline_story else "[baseline] No rewritten story produced.")
        ...
        vb_baseline = baseline_eval["violations"]
        baseline_resolution = ((vb - vb_baseline) / vb) if vb else 0.0
        resolution = ((vb - va) / vb) if vb else 0.0
        print("\n=== SUMMARY ===")
        print(f"Baseline resolution: {baseline_resolution:.3f}  (gold violations {vb} → {vb_baseline})")
        print(f"Pipeline resolution: {resolution:.3f}  (gold violations {vb} → {va})")
    ...
    micro_baseline = micro_sum([r["baseline"] for r in runs_meta])
    micro_after  = micro_sum([r["after"]  for r in runs_meta])
    vb_total = sum(int(r["viol_before"]) for r in runs_meta)
    vb_baseline_total = sum(int(r["viol_baseline"]) for r in runs_meta)
    va_total = sum(int(r["viol_after"])  for r in runs_meta)
    baseline_resolution_total = ((vb_total - vb_baseline_total) / vb_total) if vb_total else 0.0
    resolution_total = ((vb_total - va_total) / vb_total) if vb_total else 0.0

    print("\n\n================== OVERALL (ALL RUNS) ==================")
    print("Micro BEFORE  :", micro_before)
    print("Micro BASELINE:", micro_baseline)
    print("Micro AFTER   :", micro_after)
    print(
        f"Total gold violations: BEFORE {vb_total} → BASELINE {vb_baseline_total} → AFTER {va_total}\n"
        f"Resolution (baseline): {baseline_resolution_total:.3f}\n"
        f"Resolution (pipeline): {resolution_total:.3f}"
    )
    print("========================================================\n")
